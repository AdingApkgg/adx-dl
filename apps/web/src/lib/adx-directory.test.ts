...
const realListingHtml = `
  <html>
    <body>
      ...
      <h1>
        <a href="/">astrodx-charts-alice.saop.cc</a>
        <span>39</span>
      </h1>
      <table>
        <tbody>
...
describe("adx directory probing", () => {
  test("builds a trailing-slash directory url", () => {
    expect(buildAdxDirectoryUrl("39")).toBe("https://astrodx-charts-alice.saop.cc/39/");
    expect(buildAdxDirectoryUrl("[X] 人マニア")).toBe(
      "https://astrodx-charts-alice.saop.cc/%5BX%5D%20%E4%BA%BA%E3%83%9E%E3%83%8B%E3%82%A2/"
    );
  });

  test("keeps only direct child files from a directory listing", () => {
    expect(
      parseAdxDirectoryFiles(listingHtml, "https://astrodx-charts-alice.saop.cc/39/")
    ).toEqual([
      { name: "bg.jpg", url: "https://astrodx-charts-alice.saop.cc/39/bg.jpg" },
      {
        name: "maidata.txt",
        url: "https://astrodx-charts-alice.saop.cc/39/maidata.txt",
      },
      {
        name: "track.mp3",
        url: "https://astrodx-charts-alice.saop.cc/39/track.mp3",
      },
    ]);
  });
  ...
  test("ignores theme and site links outside the probed directory", () => {
    expect(
      parseAdxDirectoryFiles(
        realListingHtml,
        "https://astrodx-charts-alice.saop.cc/39/"
      )
    ).toEqual([
      { name: "bg.jpg", url: "https://astrodx-charts-alice.saop.cc/39/bg.jpg" },
      {
      ...
      { name: "pv.mp4", url: "https://astrodx-charts-alice.saop.cc/39/pv.mp4" },
      {
        name: "track.mp3",
        url: "https://astrodx-charts-alice.saop.cc/39/track.mp3",
      },
  ...
  test("fetches and parses a remote directory listing", async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith("https://astrodx-charts-alice.saop.cc/39/", {
      cache: "no-store",
    });
