...
describe("SiteSettingsContent", () => {
  ...
  test("offers both maidata output toggles in every locale, with a truthful example", () => {
    const expected = {
      zh: ["谱面文件（maidata.txt）写入", "标题后附带别名", "定数保留小数"],
      en: ["Chart file (maidata.txt) output", "Append aliases to the title", "Keep decimal constants"],
      ja: ["譜面ファイル（maidata.txt）の書き出し", "タイトルに別名を付ける", "定数の小数を保持"],
    } as const;
    ...
    for (const locale of ["zh", "en", "ja"] as const) {
      ...
      // The example is generated by the real packer from the default state
      // (aliases off, constants kept), so it can never drift from behaviour.
      expect(html).toContain("&amp;title=Turn around [SD]");
      expect(html).not.toContain("(转过来");
      expect(html).toContain("&amp;lv_5=13.7");
